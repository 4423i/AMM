...
def parse_args():
    ...
    parser.add_argument('--initial', metavar='initial',dest='initial', type=int, default=10000)
...
file_name = f'data/2d/seq/{L}/{L}_{iteration}_{chunk}.dat'
os.makedirs(f'data/2d/seq/{L}', exist_ok=True)
print(jobname)
if os.path.isfile(file_name):
    os.remove(file_name)

def toppoling(h,active_sites,outflow,area,s):
    new_sites = [i for i in active_sites]
    ri = np.random.randint(low=0,high=len(active_sites))
    y,x = active_sites[ri]
    new_sites = new_sites[:ri] + new_sites[ri+1:]
    h[y,x] -= 2
    if 1 < h[y,x]:
